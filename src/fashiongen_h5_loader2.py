...
# You can keep your existing constants file if you have one.
# For this file to be self-contained, we include a minimal set here.
# If you have `constant as c` with ALIASES and BASE_MATS, import it and remove these inline constants.
# ------- Materials constants (minimal) -------
class _CStub:
    # base clothing materials to keep (common set; extend as needed)
    BASE_MATS = {
        "cotton","polyester","wool","silk","linen","viscose","rayon","nylon","acrylic","elastane",
        "spandex","lycra","modal","lyocell","cupro","triacetate","polyamide","polypropylene",
        "leather","shearling","cashmere","mohair","alpaca","down","feather","hemp","ramie"
    }
    # alias normalization for materials
    ALIASES = {
        "elastan":"elastane","elasthane":"elastane","elastodiene":"elastane","spandex":"elastane","lycra":"elastane",
        "polyamid":"polyamide","polyamde":"polyamide","poliamide":"polyamide","ny":"nylon","pnylon":"nylon",
        "cupra":"cupro","bemberg":"cupro","tri-acetate":"triacetate","tri acetate":"triacetate",
        "vicoe":"viscose","visc":"viscose","rayon-viscose":"viscose","rayon viscose":"viscose",
        "micro modal":"modal","micromodal":"modal","tencel":"lyocell",
        "calfskin":"leather","lambskin":"leather","goatskin":"leather","sheepskin":"leather","nappa leather":"leather",
        "down":"down","feathers":"feather","duck feather":"feather","goose feather":"feather",
    }
c = _CStub()
...
# ==========================
# C) Color parsing & vocab
# ==========================
# Canonical color palette + aliases; extend freely
_CANON_COLORS = {
    "black", "white", "gray", "grey", "silver", "gold",
    "red", "blue", "green", "yellow", "orange", "pink",
    "purple", "brown", "beige", "tan", "navy", "khaki",
    "cream", "ivory", "off-white", "maroon", "burgundy",
    "teal", "turquoise", "lavender", "lilac", "coral",
    "rose", "fuchsia", "magenta", "mustard", "olive",
    "lime", "cyan", "charcoal", "sand", "camel",
    "multicolor", "multi", "rainbow"
}
_COLOR_ALIASES = {
    "off white":"off-white", "offwhite":"off-white",
    "grey":"gray", "charcoal gray":"charcoal", "charcoal grey":"charcoal",
    "navy blue":"navy", "dark blue":"navy",
    "light blue":"blue", "sky blue":"blue",
    "light pink":"pink", "hot pink":"fuchsia",
    "wine":"burgundy", "oxblood":"burgundy",
    "cream":"cream", "ecru":"beige", "bone":"beige",
    "camel":"camel", "sand":"sand", "stone":"beige",
    "forest green":"green", "mint":"green", "emerald":"green",
    "mustard yellow":"mustard", "lemon":"yellow",
    "tan":"tan", "chocolate":"brown", "coffee":"brown",
    "offwhite":"off-white",
    "multi color":"multicolor", "multi-color":"multicolor", "multicolour":"multicolor",
}
# regexp catches standalone words or hyphenated combos like "off-white", "navy-blue", "red/white"
_COLOR_TOKEN_RE = re.compile(r"[a-zA-Z]+(?:-[a-zA-Z]+)?")

def _canon_color_word(w: str) -> Optional[str]:
    w = w.lower()
    w = _COLOR_ALIASES.get(w, w)
    if w in _CANON_COLORS:
        return w
    return None

def parse_colors_from_text(text: str) -> List[str]:
    """Extract a set of canonical colors from free text (e.g., product name)."""
    if not text:
        return []
    s = str(text).lower()
    # split on common color separators, keep tokens for hyphen words too
    tokens = _COLOR_TOKEN_RE.findall(s.replace("/", " ").replace("\\", " "))
    colors = []
    seen = set()
    for tok in tokens:
        tok = _COLOR_ALIASES.get(tok, tok)
        if tok in _CANON_COLORS and tok not in seen:
            colors.append(tok); seen.add(tok)
        # also try splitting hyphen tokens: e.g., "off-white" -> ["off", "white"] then alias "off white"
        if "-" in tok:
            parts = tok.split("-")
            join2 = " ".join(parts)
            alias = _COLOR_ALIASES.get(join2, join2)
            cw = _canon_color_word(alias)
            if cw and cw not in seen:
                colors.append(cw); seen.add(cw)
            else:
                for p in parts:
                    cw2 = _canon_color_word(p)
                    if cw2 and cw2 not in seen:
                        colors.append(cw2); seen.add(cw2)
    # special handling: if multiple distinct colors → include "multicolor" if present
    if len(colors) >= 3 and "multicolor" not in colors and "multi" not in colors:
        colors.append("multicolor")
    return colors

def build_color_vocab_from_h5(
    h5_path: str,
    *,
    name_key: str = "input_name",
    caption_fallback_keys: Tuple[str, ...] = ("input_description","input_concat_description","caption","descriptions"),
    min_count: int = 25,
    top_k: Optional[int] = None,
    add_other: bool = True,
    out_json: str = "color_vocab.json",
) -> Dict[str, int]:
    cnt = Counter()
    with h5py.File(h5_path, "r") as f:
        # Prefer product name; fall back to captions if needed
        target_key = name_key if name_key in f else next((k for k in caption_fallback_keys if k in f), None)
        if target_key is None:
            raise KeyError(f"No '{name_key}' or caption fallback keys found in {list(f.keys())}")
        d = f[target_key]
        N = d.shape[0]
        for i in range(N):
            raw = _as_scalar(d[i])
            text = _decode_bytes(raw)
            cols = parse_colors_from_text(text)
            for c_ in cols:
                cnt[c_] += 1
    ...
    print(f"[color_vocab] kept {len(vocab)} tokens → {out_json}")
    return vocab

def encode_colors_multi_hot(texts: Iterable[str], vocab: Dict[str,int]) -> np.ndarray:
    V = len(vocab)
    arr = np.zeros((len(texts), V), dtype=np.float32)
    for i, t in enumerate(texts):
        cols = parse_colors_from_text(t)
        hit = False
        for col in cols:
            j = vocab.get(col, vocab.get("__other__"))
            if j is not None:
                arr[i, j] = 1.0
                hit = True
...
# ==========================
# D) Vocab builders (category/gender/material)
# ==========================
...
class FashionGenH5(Dataset):
    ...
    Returns dict:
      ...
      caption: str  (can be input_name if prefer_name_for_caption=True)
      name_raw: str (optional, always surfaced if present and emit_name_field=True)
      materials: np.ndarray [V_mat] (multi-hot) if material_vocab provided, else None
      colors: np.ndarray [V_color] (multi-hot) if color_vocab provided, else None
      meta:    dict[str, Any]
      index:   int row index
    """
    def __init__(
        ...
        # Colors
        color_vocab_json: Optional[str] = None,
        color_from_key: str = "input_name",  # which H5 key to parse colors from (default: product name)
        # Metadata
        meta_keys: Tuple[str, ...] = (),
        meta_prefix: str = "",
        # Name-as-caption switch
        prefer_name_for_caption: bool = False,
        name_key: str = "input_name",
        emit_name_field: bool = True,
        ...
        self.prefer_name_for_caption = prefer_name_for_caption
        self.name_key = name_key
        self.emit_name_field = emit_name_field
        self.color_from_key = color_from_key
        ...
        if color_vocab_json and Path(color_vocab_json).exists():
            with open(color_vocab_json) as cf:
                self.color_vocab = json.load(cf)
        ...
        with h5py.File(self.h5_path, "r") as f:
            ...
            if self.prefer_name_for_caption and (self.name_key in f):
                self.caption_key = self.name_key
            else:
            ...
            self.has_name = (self.name_key in f)
            self.has_color_source = (self.color_from_key in f)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # product name (raw)
        name_raw = ""
        if self.emit_name_field and self.has_name:
            name_val = _as_scalar(f[self.name_key][real_i])
            name_raw = _decode_bytes(name_val)
        ...
        # colors (multi-hot) from name (default) or any key you choose
        color_vec = None
        if self.color_vocab is not None and self.has_color_source:
            color_text = _decode_bytes(_as_scalar(f[self.color_from_key][real_i]))
            cols = parse_colors_from_text(color_text)
            Vc = len(self.color_vocab)
            cv = np.zeros((Vc,), dtype=np.float32)
            hitc = False
            for c_ in cols:
                j = self.color_vocab.get(c_, self.color_vocab.get("__other__"))
                if j is not None:
                    cv[j] = 1.0
                    hitc = True
            if not hitc and "__other__" in self.color_vocab:
                cv[self.color_vocab["__other__"]] = 1.0
            color_vec = cv
...
def make_datasets_from_h5(
    ...
    *,
    ...
    train_ds = FashionGenH5(
        ...
        color_vocab_json=color_vocab_json, color_from_key=color_from_key,
        meta_keys=meta_keys, meta_prefix=meta_prefix,
        prefer_name_for_caption=prefer_name_for_caption, name_key=name_key, emit_name_field=emit_name_field,
